...
3. `python-consumer.yml` downloads that artifact and runs Python inference in separate CI tasks for ONNX and pickle, plus separate iris-example CI tasks for ONNX and pickle.
4. `java-consumer.yml` does the same for Java inference tests, plus a separate iris-example CI task.
5. `js-consumer.yml` does the same for JavaScript inference tests, plus a separate iris-example CI task.
